...
var ReactVersion = '19.1.0-experimental-e06c72fc-20241215';
...
var COMPONENTS_TRACK = 'Server Components ⚛'; // Reused to avoid thrashing the GC.
...
function logComponentRender(componentInfo, startTime, endTime, childrenEndTime) {
  if (supportsUserTiming && childrenEndTime >= 0) {
    var name = componentInfo.name;
    var selfTime = endTime - startTime;
    reusableComponentDevToolDetails.color = selfTime < 0.5 ? 'primary-light' : selfTime < 50 ? 'primary' : selfTime < 500 ? 'primary-dark' : 'error';
...
function flushComponentPerformance(root) {
  ...
  if (debugInfo) {
    ...
    for (var _i3 = debugInfo.length - 1; _i3 >= 0; _i3--) {
      ...
      if (typeof info.name === 'string' && _i3 > 0) {
        // $FlowFixMe: Refined.
        var componentInfo = info;
        var startTimeInfo = debugInfo[_i3 - 1];

        if (typeof startTimeInfo.time === 'number') {
          var startTime = startTimeInfo.time;
          logComponentRender(componentInfo, startTime, endTime, childrenEndTime);
        }
      }
    }
  }
