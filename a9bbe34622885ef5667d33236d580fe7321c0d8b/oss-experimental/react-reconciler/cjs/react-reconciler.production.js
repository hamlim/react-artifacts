  ...
  function completeWork(current, workInProgress, renderLanes) {
    ...
    switch (workInProgress.tag) {
      ...
      case 13:
        ...
        if (renderLanes) {
          ...
          null !== newProps.memoizedState &&
            ...
            (cache$114 = newProps.memoizedState.cachePool.pool);
          cache$114 !== nextResource && (newProps.flags |= 2048);
        }
        renderLanes !== current &&
          renderLanes &&
      ...
      case 19:
        ...
        cache$114 = nextResource.rendering;
        if (null === cache$114)
          if (newProps) cutOffTailIfNeeded(nextResource, !1);
            ...
            if (
              ...
              for (current = workInProgress.child; null !== current; ) {
                cache$114 = findFirstSuspended(current);
                if (null !== cache$114) {
                  workInProgress.flags |= 128;
                  cutOffTailIfNeeded(nextResource, !1);
                  current = cache$114.updateQueue;
                  workInProgress.updateQueue = current;
          ...
          if (!newProps)
            if (((current = findFirstSuspended(cache$114)), null !== current)) {
              if (
                ((workInProgress.flags |= 128),
                (newProps = !0),
                (current = current.updateQueue),
          ...
          nextResource.isBackwards
            ? ((cache$114.sibling = workInProgress.child),
              (workInProgress.child = cache$114))
            : ((current = nextResource.last),
              null !== current
                ? (current.sibling = cache$114)
                : (workInProgress.child = cache$114),
              (nextResource.last = cache$114));
        }
      ...
      if (null !== finishedWork) {
        ...
        if (0 !== (finishedWork.subtreeFlags & 15990) || rootHasEffect) {
          ...
          for (nextEffect = finishedWork; null !== nextEffect; ) {
            rootHasEffect = nextEffect;
            var child = rootHasEffect.child;
            if (0 !== (rootHasEffect.subtreeFlags & 1028) && null !== child)
              (child.return = rootHasEffect), (nextEffect = child);
            else
              b: for (; null !== nextEffect; ) {
                rootHasEffect = nextEffect;
                updatedLanes = void 0;
                suspendedRetryLanes = rootHasEffect;
                var current = suspendedRetryLanes.alternate,
                  flags = suspendedRetryLanes.flags;
                switch (suspendedRetryLanes.tag) {
                  case 0:
                    if (0 !== (flags & 4)) {
                      var updateQueue = suspendedRetryLanes.updateQueue,
                        eventPayloads =
                  ...
                  case 1:
                    if (0 !== (flags & 1024) && null !== current) {
                      var prevProps = current.memoizedProps,
                        ...
                        instance = suspendedRetryLanes.stateNode;
                      try {
                        var resolvedPrevProps = resolveClassComponentProps(
                          suspendedRetryLanes.type,
                          prevProps,
                          suspendedRetryLanes.elementType ===
                            suspendedRetryLanes.type
                        );
                        updatedLanes = instance.getSnapshotBeforeUpdate(
                          resolvedPrevProps,
                          prevState
                        );
                  ...
                  case 3:
                    0 !== (flags & 1024) &&
                      supportsMutation &&
                      clearContainer(
                        suspendedRetryLanes.stateNode.containerInfo
                      );
                    break;
                  case 5:
                  case 26:
                ...
                var sibling = rootHasEffect.sibling;
                if (null !== sibling) {
                  sibling.return = rootHasEffect.return;
                  nextEffect = sibling;
                  break b;
                }
                nextEffect = rootHasEffect.return;
              }
          }
