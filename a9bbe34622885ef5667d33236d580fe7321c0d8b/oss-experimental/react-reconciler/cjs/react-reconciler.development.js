  ...
  ((module.exports = function ($$$config) {
    ...
    function commitRoot(
      ...
      try {
        ...
        if (null !== finishedWork) {
          ...
          completedRenderStartTime = remainingLanes |= concurrentlyUpdatedLanes;
          var previouslyPendingLanes = root$jscomp$0.pendingLanes;
          root$jscomp$0.pendingLanes = completedRenderStartTime;
          root$jscomp$0.suspendedLanes = 0;
          root$jscomp$0.pingedLanes = 0;
          root$jscomp$0.warmLanes = 0;
          root$jscomp$0.expiredLanes &= completedRenderStartTime;
          root$jscomp$0.entangledLanes &= completedRenderStartTime;
          root$jscomp$0.errorRecoveryDisabledLanes &= completedRenderStartTime;
          ...
          for (
            completedRenderStartTime =
              previouslyPendingLanes & ~completedRenderStartTime;
            0 < completedRenderStartTime;

          ) {
            var index = 31 - clz32(completedRenderStartTime);
            hydrationFailed = 1 << index;
            entanglements[index] = 0;
            expirationTimes[index] = -1;
            var hiddenUpdatesForLane = hiddenUpdates[index];
            if (null !== hiddenUpdatesForLane)
              for (
                hiddenUpdates[index] = null, error = 0;
                error < hiddenUpdatesForLane.length;
                error++
              ) {
                var update = hiddenUpdatesForLane[error];
                null !== update && (update.lane &= -536870913);
              }
            completedRenderStartTime &= ~hydrationFailed;
          }
          0 !== spawnedLane &&
          ...
          if (0 !== (finishedWork.subtreeFlags & 15990) || rootHasEffect) {
            ...
            prepareForCommit(root$jscomp$0.containerInfo);
            for (nextEffect = finishedWork; null !== nextEffect; ) {
              transitions = nextEffect;
              var child = transitions.child;
              if (0 !== (transitions.subtreeFlags & 1028) && null !== child)
                (child.return = transitions), (nextEffect = child);
              else
                b: for (; null !== nextEffect; ) {
                  updatedLanes = transitions = nextEffect;
                  var current = updatedLanes.alternate,
                    flags = updatedLanes.flags;
                  switch (updatedLanes.tag) {
                    case 0:
                      if (0 !== (flags & 4)) {
                        var updateQueue = updatedLanes.updateQueue,
                          eventPayloads =
                            null !== updateQueue ? updateQueue.events : null;
                        if (null !== eventPayloads)
                          for (
                            updatedLanes = 0;
                            updatedLanes < eventPayloads.length;
                            updatedLanes++
                          ) {
                            var _eventPayloads$ii = eventPayloads[updatedLanes];
                    ...
                    case 1:
                      0 !== (flags & 1024) &&
                        null !== current &&
                        commitClassSnapshot(updatedLanes, current);
                      break;
                    case 3:
                      0 !== (flags & 1024) &&
                        supportsMutation &&
                        clearContainer(updatedLanes.stateNode.containerInfo);
                      break;
                    case 5:
                    case 26:
                    case 27:
                  ...
                  var sibling = transitions.sibling;
                  if (null !== sibling) {
                    sibling.return = transitions.return;
                    nextEffect = sibling;
                    break b;
                  }
                  nextEffect = transitions.return;
                }
            }
            ...
            resetAfterCommit(root$jscomp$0.containerInfo);
    ...
    exports.injectIntoDevTools = function () {
      var internals = {
        ...
        reconcilerVersion: "19.1.0-experimental-a9bbe346-20241219"
      };
